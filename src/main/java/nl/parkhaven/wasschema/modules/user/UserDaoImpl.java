...
public class UserDaoImpl implements Crud<User> {
  ...
  public boolean updateRememberLaundryRoom(User user) {
    int rowsAffected = this.template
        .update(UPDATE_REMEMBER_LAUNDRY_ROOM, user.isRememberLaundryRoomChecked(), user.getId());
    return rowsAffected == 1;
  }
  ...
  public boolean houseNumberExist(User user) {
    return this.template
        .queryForObject(CHECK_HOUSE_NUMBER_EXISTS, new Object[]{user.getHouseNumber()},
            Boolean.class);
  }
