...
public class LoginControllerTest {
  ...
  public void testLogin() throws Exception {
    ...
    when(loginService.isLoginCredentialsValid(ArgumentMatchers.any(User.class))).thenReturn
        (true);
    ...
    RequestBuilder request = post("/").param("to_servlet", "login").param("email",
        "email@gmail.com").param("password", "password");

    mockMvc.perform(request).andDo(MockMvcResultHandlers.print()).andExpect(status()
        .is3xxRedirection()).andExpect(redirectedUrl("/index.010/loggedin"));
  ...
  public void testSignup() throws Exception {
    RequestBuilder request = post("/")
        .param("to_servlet", "signup").param("email",
            "email@gmail.com").param("password", "password").param("houseNumber", "230")
        .param("sharedPassword", "3024NH");

    when(loginService.signupCredentialsValid(ArgumentMatchers.any(User.class))).thenReturn
        (true);
  ...
  public void testForgotPassword() throws Exception {
    RequestBuilder request = post("/").param("to_servlet", "forgotPassword").param("email",
        "email@gmail.com");
